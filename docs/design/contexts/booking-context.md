...
# 1. 目的
Bookingコンテキストは、ユーザーがサービスを予約するための境界コンテキストです。このコンテキストは、予約の作成、変更、キャンセルを管理し、予約の衝突を防止する機能を提供します。
...
[Booking] --(checks)--> [Slot]
[Booking] --(detects)--> [Conflict]
[Booking] --(issues)--> [BookingCreatedEvent]
```
...
# 6. ドメインイベント
- `BookingCreatedEvent`: 予約作成時
- `BookingUpdatedEvent`: 予約変更時
- `BookingCancelledEvent`: 予約キャンセル時
- `ConflictDetectedEvent`: 衝突検出時

# 7. 非機能（SLO/Obs/Sec）
- **SLO**: 予約作成APIの可用性99.9%
- **Obs**: 予約作成・変更・キャンセルの全操作のトレースとメトリクス収集
- **Sec**: 予約情報のセキュリティ保護、衝突防止のための同時実行制御
